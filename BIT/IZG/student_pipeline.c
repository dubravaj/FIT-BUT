...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  /// Správná adresa se odvíjí od adresy bufferu, offsetu čtěcí hlavy, čísla vrcholu a kroku čtecí hlavy.
    ...
    return (char *)head->buffer + head->offset + (head->stride * gl_VertexID);   
...
void gpu_runVertexPuller(
  ...
  GPUVertexPullerHead const *head;
  VertexIndex const *indices;
  void const *data_attribute_pointer = NULL;
  VertexIndex gl_VertexID;
  
  for(size_t i = 0; i < 4; i++){
     head = &puller->heads[i];
     if(head->enabled){
          indices = puller->indices;
          gl_VertexID = gpu_computeGLVertexID(indices,vertexShaderInvocation);
          data_attribute_pointer = gpu_computeVertexAttributeDataPointer(head,gl_VertexID); 
          output->attributes[i] = data_attribute_pointer;
     }
     else{
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float dist1 = distanceTo2DLine(&lines[1],pixelCenter) / distanceTo2DLine(&lines[1],&vertices[0]);
  float dist2 = distanceTo2DLine(&lines[2],pixelCenter) / distanceTo2DLine(&lines[2],&vertices[1]);
  coords->data[0] = dist1;
  coords->data[1] = dist2;
  coords->data[2] = 1.f - dist1 - dist2;
