...
**/
    tBTNodePtr tmp_node_first = NULL;
    tBTNodePtr tmp_node_second = NULL;
    tmp_node_first = malloc(sizeof(struct tBTNode));
    if(tmp_node_first == NULL){
        fprintf(stderr,"Chyba pri alokaci pamate.\n");
        exit(1);
    }
    tmp_node_first->Cont = Content;
    tmp_node_first->LPtr = NULL;
    tmp_node_first->RPtr = NULL;

   if((*RootPtr) == NULL){
        (*RootPtr) = tmp_node_first;         
   }
   else{//vladanie uzlu 
        tmp_node_second = (*RootPtr);
        while(1){
            if(tmp_node_second->Cont == Content){
                return;
            }//vkladanie uzlu s vacsou hodnotou ako koren
            else if(tmp_node_second->Cont < Content){
                    if(tmp_node_second->RPtr == NULL){
                        tmp_node_second->RPtr = tmp_node_first;
                        return;
                    }
                    else{
                        tmp_node_second = tmp_node_second->RPtr;
                    }
            }
            else{//vkladanie uzlu s mensou hodnotou ako koren
                if(tmp_node_second->LPtr == NULL){
                    tmp_node_second->LPtr = tmp_node_first;
                    return;
                }
                else{
                    tmp_node_second = tmp_node_second->LPtr;
                }
            }
        }
...
**/
    ...
    SInitB(&Stack_bool);
    tmp = RootPtr;
    Leftmost_Postorder(tmp,&Stack,&Stack_bool); 
    
    while(!(SEmptyP(&Stack))){
        tmp = STopPopP(&Stack);
        SPushP(&Stack,tmp);
        tmp_bool = STopPopB(&Stack_bool);
        if(tmp_bool){
            SPushB(&Stack_bool,FALSE);
            tmp = tmp->RPtr;
            Leftmost_Postorder(tmp,&Stack,&Stack_bool);
        }
        else{
...
**/
    ...
    if((*RootPtr) == NULL){
        return;
         ...
         do{//postupne rusime uzly stromu
            if((*RootPtr) == NULL){
                if(!(SEmptyP(&Stack))){
                    (*RootPtr) = STopPopP(&Stack);
                }
            }
            else{//ide sa dolava a prave ukazatele sa ukladaju na zasobnik
                if((*RootPtr)->RPtr != NULL){
                    SPushP(&Stack,(*RootPtr)->RPtr);
                }
                tBTNodePtr node_tmp = NULL;
                tmp = (*RootPtr); //uchovanie korena
                node_tmp = (*RootPtr)->LPtr;  //posunieme sa dolava
                free(tmp); //uvolnime koren
                (*RootPtr) = node_tmp;
