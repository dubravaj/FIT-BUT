...
void phong_onInit(int32_t width,int32_t height){
  ...
  cpu_bufferData(phong.gpu,phong.vbo,sizeof(struct BunnyVertex) * 1048 ,bunnyVertices);
  
  cpu_createBuffers(phong.gpu,1,&phong.ebo);
  cpu_bufferData(phong.gpu,phong.ebo,sizeof(VertexIndex) * 2092 * 3, bunnyIndices);
  
  cpu_createVertexPullers(phong.gpu,2,&phong.pullerID);
  
  size_t stride = sizeof(VertexIndex) * 3;
  cpu_setVertexPullerHead(phong.gpu,phong.pullerID,0,phong.vbo,sizeof(float)*0,stride);
  cpu_setVertexPullerHead(phong.gpu,phong.pullerID,1,phong.vbo,sizeof(float)*3,stride);

  cpu_enableVertexPullerHead(phong.gpu,phong.pullerID,0);
  cpu_enableVertexPullerHead(phong.gpu,phong.pullerID,1);

  cpu_setIndexing(phong.gpu,phong.pullerID,phong.ebo,1);
