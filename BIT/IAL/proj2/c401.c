...
**/
    ...
    if(RootPtr != NULL){
        ...
        else{//na zaklade hodnoty kluca prehladavame bud vlavo alebo vpravo
            if(RootPtr->Key > K){
...
**/
    ...
    if((*RootPtr) == NULL){//vytvorenie noveho uzlu
        tBSTNodePtr node = malloc(sizeof(struct tBSTNode));
        if(node == NULL){
            return;
        } 
        node->Key = K;
...
**/
   // solved = FALSE;
    ...
    if((*RootPtr)->RPtr != NULL){
        ...
        tBSTNodePtr tmp_node = NULL;
        tmp_node = (*RootPtr);
        PtrReplaced->BSTNodeCont = tmp_node->BSTNodeCont;
        PtrReplaced->Key = tmp_node->Key;
        PtrReplaced = tmp_node; 
        (*RootPtr) = tmp_node->LPtr;
        free(tmp_node);
        return;
    }	
	return;
